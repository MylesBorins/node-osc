import { fromBuffer } from '../osc-min-wrapper.mjs';

function sanitizeMessage(decoded) {
  const message = [];
  message.push(decoded.address);
  decoded.args.forEach(arg => {
    // Convert DataView blobs back to Buffer for compatibility
    if (arg.value instanceof DataView) {
      message.push(Buffer.from(arg.value.buffer, arg.value.byteOffset, arg.value.byteLength));
    } else {
...
function decode(data) {
  ...
  else { // oscType === 'message' - the only other possibility according to osc-min types
