...
function TInt64 (value) { this.value = value; }
TInt64.prototype = {
    typetag: 'h',
    decode: function (data) {
        if (data.length < 8) {
            throw new ShortBuffer('int64', data, 8);
        }

        var int = jspack.Unpack('>q', data.slice(0, 8))[0];
        this.value = (int[1] << 8) + int[0];

        return data.slice(8);
    },
    encode: function (buf, pos) {
        return pack('>q', buf, pos, [ this.value ]);
    }
};

exports.TInt64 = TInt64;
