    ...
    for (var i = 1; i < arguments.length; i++) {
        ...
        case 'object':
            if (arg.typetag) {
                ...
                throw "don't know how to encode object " + arg;
            }
            break;
        case 'number':
        ...
        case 'string':
            ...
            break;
        default:
            throw "don't know how to encode " + arg;
        }
    }
}
...
TString.prototype = {
    ...
    decode: function (data) {
        ...
        this.value = data.substring(0, length);
        var nextData = parseInt(Math.ceil((length + 1) / 4.0) * 4);
        return data.substr(nextData);
    },
...
TInt.prototype = {
    ...
    decode: function (data) {
        if (data.length < 4) {
            throw 'too few bytes for int ' + data + data.length;
        }

        this.value = jspack.Unpack('>i', new Buffer(data.substring(0, 4))) || 0;
...
TTime.prototype = {
    ...
    decode: function (data) {
        if (data.length < 8) {
            throw 'too few bytes for time ' + data + data.length;
        }
        this.value = jspack.Unpack('>LL', new Buffer(data.substring(0, 8)));
...
TFloat.prototype = {
    ...
    decode: function (data) {
        if (data.length < 4) {
            throw 'too few bytes for float ' + data + data.length;
        }

        this.value = jspack.Unpack('>f', new Buffer(data.substring(0, 4))) || 0;
        return data.substr(4);
    },
    encode: function (buf, pos) {
...
TBlob.prototype = {
    ...
    decode: function (data) {
        ...
        this.value = data.substring(4, length + 4);
        return data.substr(nextData);
    },
    encode: function (buf, pos) {
...
TDouble.prototype = {
    ...
    decode: function (data) {
        if (data.length < 8) {
            throw 'too few bytes for double ' + data + data.length;
        }
        this.value = jspack.Unpack('>d', new Buffer(data.substring(0, 8))) || 0;
        return data.substr(8);
    },
    encode: function (buf, pos) {
